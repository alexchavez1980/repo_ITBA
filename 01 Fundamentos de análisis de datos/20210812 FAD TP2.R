...
library(ggplot2) #Para el boxplot/scatterplot
library(tidyverse)
library(ggcorrplot)
library(stats)
library(MASS)
...
matrizS <- matrix(c(3,1,1,1,3,1,1,1,5),nrow=3)
aut <- eigen(matrizS)
aut
#Los autovectores te los da en columnas
autovalores=eigen(matrizS)$values
autovalores
autovectores=eigen(matrizS)$vectors
autovectores
...
#Para ubicar la ruta y almacenarla en una variable.
file.choose()
rutads<-"D:\\Alex\\Estudio\\Esp Ciencia de Datos\\01 Fundamentos de análisis de datos\\TP2\\europe.csv"
#-----------------------------------------------------
#Punto 2
#Lectura/Carga y almacentamiento del dataset en dataset_FAD_TP2
#-----------------------------------------------------
ds_tp2<-read.csv2(rutads)
View(ds_tp2)
#No tiene registros con NA.

#Agrego columnas con cada una de las medias de las variables.
ds_tp2$mean_Area <- mean(ds_tp2$Area)
ds_tp2$mean_GDP	<- mean(ds_tp2$GDP)
ds_tp2$mean_Inflation	<- mean(ds_tp2$Inflation)
ds_tp2$mean_Life.expect	<- mean(ds_tp2$Life.expect)
ds_tp2$mean_Military <- mean(ds_tp2$Military)
ds_tp2$mean_Pop.growth <- mean(ds_tp2$Pop.growth)
ds_tp2$mean_Unemployment <- mean(ds_tp2$Unemployment)
View(ds_tp2)
...
#Acá selecciono las variables dónde aplicar la distancia de Mahalanobis
